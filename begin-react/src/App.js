...
function Counter(){
	...
	<fieldset>
		...
		<div><button onClick={increaseCount}>+</button><button onClick={decreaseCount}>-</button></div>
	</fieldset>
	);
}
...
return(
	<div style={style}>
		<div><input type="text" onChange={transport} value={text}/><button type="button" onClick={Initialize}>initialize</button></div>
...
function MultiInput(){
	...
	return(
		<fieldset>
			...
			<div>
				<input type="text" name="name" onChange={onchange} value={name} ref={nameInput} placeholder="Name" />
				<input type="text" name="nickname" onChange={onchange} value={nickname} placeholder="Nickname"/>
				<button type="button" onClick={onReset}>Reset</button>
			</div>
			<br/>
			<div><span>{name}</span> / <span>{nickname}</span></div>
			<br/>
		</fieldset>
	);
}
...
function List({parm, onremove, ontoggle}){
	...
	return(
		<div>
			<strong style={style} onClick={()=>ontoggle(id)}>{account_user}</strong> / <span>{email}</span>
...
function CreateList({onchange, oncreate, onreset, user, email, firstFocus}){
	return(
		<div>
			<input type="text" onChange={onchange} name="user" value={user} ref={firstFocus}/>
			<input type="text" onChange={onchange} name="email" value={email}/>
			<button onClick={()=>{oncreate();onreset();}}>Regist</button>
		</div>
	);
}
